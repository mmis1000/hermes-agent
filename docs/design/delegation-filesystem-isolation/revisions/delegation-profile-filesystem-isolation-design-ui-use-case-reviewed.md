...
```text
-v /work/ui-runs/job-123/handoffs/builder-1:/work/ui-runs/job-123/handoffs/builder-1:ro
```
...
```json
{
  ...
  "profile": "ui-isolated",
  "workdir": "/work/ui-runs/job-123/builder-workspaces/builder-1",
  "reveal": [
    {
      "path": "/work/ui-runs/job-123/handoffs/builder-1",
      "mode": "ro"
    ...
    {
      "path": "/work/ui-runs/job-123/builder-workspaces/builder-1",
      "mode": "rw"
    }
  ]
}
...
The initial design uses the smallest useful profile set: one general isolated profile and one browser-ready variant. UI roles do not automatically require separate profiles when different explicit reveal sets provide the needed filesystem lanes. Additional profiles require an actual difference in execution policy, not merely a role name.
...
If an orchestrator passes prohibited information through text or reveals a semantically incorrect but technically authorized packet, existing workflow contamination rules apply. The runtime does not attempt to replace those rules with a new policy engine.
