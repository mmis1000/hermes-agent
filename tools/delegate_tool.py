...
def _parent_finalization_lock(parent_agent):
...
def _finalize_child_results(results, task_list, children, parent_agent) -> None:
    """Apply the host-owned summary, memory, hook, and cost contracts once."""
    with _parent_finalization_lock(parent_agent):
            ...
            for entry in results:
                try:
                    ...
                    goal = (
                        ...
                        if isinstance(index, int) and 0 <= index < len(task_list)
                        else ""
                    )
                    memory_manager.on_delegation(
                        ...
                        child_session_id=getattr(
                            child_by_index.get(index), "session_id", ""
                        ),
                    )
                except Exception:
                    pass
        ...
        for entry in results:
            ...
            try:
                children_cost_total += float(child_cost or 0.0)
            except (TypeError, ValueError):
                pass
            ...
            try:
                ...
                invoke_hook(
                    ...
                    duration_ms=int(
                        (entry.get("duration_seconds") or 0) * 1000
                    ),
                )
            except Exception:
                logger.debug("subagent_stop hook invocation failed", exc_info=True)

        if children_cost_total > 0:
            try:
                current = float(
                    getattr(parent_agent, "session_estimated_cost_usd", 0.0) or 0.0
                )
                parent_agent.session_estimated_cost_usd = (
                    current + children_cost_total
                )
                ...
                if msg.get("role") == "assistant":
                    for tc in msg.get("tool_calls") or []:
                        ...
                        entry_t = {
                            ...
                            "input_summary": _summarize_tool_arguments(fn.get("arguments", "")),
                        }
